...
public interface RoleMapper {
	...
	@Select(value={"select * from t_role where name like CONCAT(#{roleName},'%') and role_type like CONCAT(#{roleType})"})
	@Results({
		@Result(id=true,property="id",column="id",javaType=Long.class),
		@Result(property="name",column="name",javaType=String.class),
		@Result(property="roleType",column="role_type",javaType=String.class)
	})
	public List<Role> searchRole(@Param("roleName")String roleName,@Param("roleType")String roleType);
