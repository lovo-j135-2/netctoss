...
public interface OperateLogMapper {
	...
	//传统分页查询！！
	@Select(value="select * from t_operatelog where account like CONCAT(#{account},'%') and op_type like CONCAT(#{type},'%') and (op_time between #{beginTime} and #{endTime}) limit #{op.index},#{op.lines}")
	@Results({
		@Result(id=true,property="id",column="id",javaType=Long.class),
		@Result(property="account",column="account",javaType=String.class),
		@Result(property="operateType",column="op_type",javaType=String.class),
	...
	public List<OperateLogBean> findOperateLogBeanByTimeToPage(@Param("op")OperateLogPageBean op,@Param("account")String account,@Param("type")String type,@Param("beginTime")String beginTime,@Param("endTime")String endTime) throws Exception;
	
	@Select(value="select count(*) from t_operatelog where account like CONCAT(#{account},'%') and op_type like CONCAT(#{type},'%') and (op_time between #{beginTime} and #{endTime})")
	public int findCountOperateLogBeanByTimeToPage(@Param("account")String account,@Param("type")String type,@Param("beginTime")String beginTime,@Param("endTime")String endTime) throws Exception;
